...
export default async function ProximoEventoPage() {
  ...
  const { data: setlistSongsData } = nextEvent
    ? await supabase
        .from('setlist_songs')
        .select(
          'id, song_id, song_title, artist, key_note, moment, soloist_id, version, reference_link, profiles(full_name)'
        )
        ...
        {!nextEvent ? (
          <div className="flex flex-col items-center justify-center py-20 text-center border border-white/[0.06] rounded-modal">
            ...
            <p className="text-sm text-[#64748B] mt-1">
              Assim que um culto for criado na Agenda, as músicas aparecerão aqui.
            </p>
          </div>
          ...
          <div className="flex flex-col items-center justify-center py-20 text-center border border-white/[0.06] rounded-modal">
            ...
            <p className="text-sm text-[#64748B] mt-1">
              As músicas adicionadas ao evento aparecerão aqui para estudo.
            </p>
          </div>
        ) : (
                    ...
                    <div className="min-w-0 flex-1">
                      ...
                      <div className="mt-3 flex flex-wrap items-center gap-2">
                        ...
                        {song.profiles?.full_name && (
                          <span className="rounded-full bg-white/[0.04] px-2 py-1 text-xs text-[#94A3B8]">
                            {song.profiles.full_name}
                          </span>
                        )}
                      </div>
                        ...
                        return (
                          <span
                            ...
                            className={`flex-1 rounded-t-full transition-colors ${
                              active
                                ? 'bg-gradient-to-t from-emerald-500 via-cyan-300 to-white shadow-[0_0_12px_rgba(34,211,238,0.35)]'
                                : 'bg-white/[0.06]'
                            }`}
...
async function loadStemsForSongs(
  ...
  for (const song of songs) {
    const stemsForSong = allStems.filter(
      (stem) =>
        stem.setlist_song_id === song.id ||
        (song.song_id && stem.song_id === song.song_id)
    )
