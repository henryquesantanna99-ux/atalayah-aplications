...
export async function getMusicasParaVotacao(): Promise<VotableSong[]> {
  ...
  if (managedSongs && managedSongs.length > 0) return managedSongs as unknown as VotableSong[]
  ...
  return ((catalog ?? []) as unknown as CatalogSongRow[]).map((item) => ({
    id: item.id,
    song_title: item.songs?.title ?? 'Música sem título',
    artist: item.artist ?? item.songs?.artist ?? null,
    youtube_link: item.youtube_url ?? item.songs?.youtube_url ?? null,
    category: item.moment === 'Palavra' ? 'Adoração' : item.moment,
    status: 'Repertório oficial',
    theme: null,
    worship_type: 'Ambos',
    votes: 0,
    average_rating: null,
