...
export function WorshipVotingClient({ songs }: { songs: Song[] }) {
  ...
  const [suggestion, setSuggestion] = useState({
    nome: '', tribo: '', telefone: '', musica: '', artista: '', youtubeLink: '', categoriaSugerida: '', tipoLouvor: '', motivo: '',
  })
  ...
  function submitSuggestion(event: React.FormEvent) {
    ...
    startTransition(async () => {
      ...
      if (response.success) {
        ...
        setSuggestion({ nome: '', tribo: '', telefone: '', musica: '', artista: '', youtubeLink: '', categoriaSugerida: '', tipoLouvor: '', motivo: '' })
        setView('home')
      } else {
  ...
  if (view === 'suggest') {
    return <section className="max-w-3xl mx-auto rounded-2xl border border-white/[0.08] bg-navy-900 p-5 sm:p-6">
      ...
      <form onSubmit={submitSuggestion} className="grid gap-4">
        ...
        <div><Label>Por que você está indicando essa música?</Label><Textarea className="mt-2 min-h-28 bg-black/20 border-white/10 text-white" value={suggestion.motivo} onChange={(e) => setSuggestion({ ...suggestion, motivo: e.target.value })} /></div>
  ...
  if (view === 'vote') {
    return <section className="space-y-5">
      ...
      {voteSong && <div className="fixed inset-0 z-50 bg-black/80 p-4 overflow-y-auto"><form onSubmit={submitVote} className="mx-auto mt-8 max-w-lg rounded-2xl border border-white/10 bg-navy-900 p-5 space-y-4"><BackButton onClick={() => setVoteSong(null)} /><h3 className="text-xl font-bold text-white">Votar em {getTitle(voteSong)}</h3><Field label="Nome *" value={voteForm.nome} onChange={(nome) => setVoteForm({ ...voteForm, nome })} /><Field label="Telefone / WhatsApp *" value={voteForm.telefone} onChange={(telefone) => setVoteForm({ ...voteForm, telefone })} inputMode="tel" /><Field label="Tribo / Grupo / Ministério" value={voteForm.tribo} onChange={(tribo) => setVoteForm({ ...voteForm, tribo })} /><SelectField label="Você conhece essa música?" value={voteForm.conheceMusica} options={['Sim', 'Não']} onChange={(conheceMusica) => setVoteForm({ ...voteForm, conheceMusica })} /><SelectField label="Essa música ajuda você a cantar junto no culto?" value={voteForm.ajudaACantar} options={['Sim', 'Não', 'Não sei']} onChange={(ajudaACantar) => setVoteForm({ ...voteForm, ajudaACantar })} /><SelectField label="Nota" value={voteForm.nota} options={['1', '2', '3', '4', '5']} onChange={(nota) => setVoteForm({ ...voteForm, nota })} /><Button disabled={isPending} className="w-full h-12 bg-brand hover:bg-brand/90"><Vote className="w-4 h-4" />{isPending ? 'Registrando...' : 'Registrar voto'}</Button></form></div>}
    </section>
  }
...
function Field({ label, value, onChange, ...props }: { label: string; value: string; onChange: (value: string) => void } & Omit<React.InputHTMLAttributes<HTMLInputElement>, 'value' | 'onChange'>) { return <div><Label>{label}</Label><Input {...props} className="mt-2 h-11 bg-black/20 border-white/10 text-white" value={value} onChange={(e) => onChange(e.target.value)} /></div> }
function SelectField({ label, value, options, onChange }: { label: string; value: string; options: string[]; onChange: (value: string) => void }) { return <div><Label>{label}</Label><Select value={value} onValueChange={onChange}><SelectTrigger className="mt-2 h-11 bg-black/20 border-white/10 text-white"><SelectValue placeholder="Selecione" /></SelectTrigger><SelectContent>{options.map((option) => <SelectItem key={option} value={option}>{option}</SelectItem>)}</SelectContent></Select></div> }
