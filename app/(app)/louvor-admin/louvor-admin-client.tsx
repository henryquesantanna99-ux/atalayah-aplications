...
type SpiritualSummary = {
  ...
  correlations?: Array<{ segment: string; value: string; dimension: string; polarity: string; segmentPercentage: number; baselinePercentage: number; difference: number; sampleSize: number; relevant: boolean }>
  interpretation?: Array<{ title: string; narrative: string; examples?: Array<{ pessoa?: string; musica?: string; evidence?: string }> }>
  actions?: Array<{ title: string; action: string; rationale: string; source: string }>
  charts_payload?: { dimensions?: Array<{ dimension: string; polarity: string; count: number; percentage: number }> }
  created_at: string
}
...
function DailySummary({ summary, activeView }: { summary: SpiritualSummary; activeView: 'panorama' | 'segmentacao' | 'associacoes' | 'evolucao' }) {
  ...
  if (activeView === 'associacoes') return <AssociationSummaryPanel items={summary.associations ?? []} correlations={summary.correlations ?? []} />
  if (activeView === 'evolucao') return <EvolutionSummaryPanel evolution={summary.evolution} />
  return <div className="space-y-5"><DimensionComposition items={summary.charts_payload?.dimensions ?? []} /><section className="grid gap-4 lg:grid-cols-2"><MetricChart title="Temas classificados" items={summary.quantification?.themes ?? []} /><MetricChart title="Necessidades declaradas" items={summary.quantification?.needs ?? []} /><MetricChart title="Emoções nas respostas" items={summary.quantification?.emotions ?? []} /><MetricChart title="Próximos passos" items={summary.quantification?.nextSteps ?? []} /></section><ExecutiveReport interpretation={summary.interpretation ?? []} actions={summary.actions ?? []} /></div>
}

function DimensionComposition({ items }: { items: NonNullable<SpiritualSummary['charts_payload']>['dimensions'] }) {
  const colors: Record<string, string> = { bem: 'bg-emerald-400', mal: 'bg-rose-400', neutro: 'bg-slate-400' }
  return <section className="rounded-2xl border border-white/[0.08] bg-navy-900 p-5"><h3 className="font-bold text-white">Composição por dimensão e polaridade</h3><p className="mt-1 text-sm text-[#94A3B8]">Percentuais calculados sobre todos os achados estruturados da classificação.</p><div className="mt-5 flex h-10 overflow-hidden rounded-xl bg-white/5">{items?.map((item) => <div key={`${item.dimension}-${item.polarity}`} title={`${item.dimension} · ${item.polarity}: ${item.percentage}%`} className={`${colors[item.polarity] || 'bg-brand'} flex min-w-8 items-center justify-center text-xs font-bold text-navy-950`} style={{ width: `${item.percentage}%` }}>{item.percentage >= 8 ? `${item.percentage}%` : ''}</div>)}</div><div className="mt-3 flex flex-wrap gap-3">{items?.map((item) => <span key={`${item.dimension}-${item.polarity}`} className="text-xs text-[#CBD5E1]">{item.dimension} · {item.polarity}: {item.count}</span>)}</div></section>
}

function ExecutiveReport({ interpretation, actions }: { interpretation: NonNullable<SpiritualSummary['interpretation']>; actions: NonNullable<SpiritualSummary['actions']> }) {
  return <section className="grid gap-4 lg:grid-cols-2"><div className="rounded-2xl border border-white/[0.08] bg-navy-900 p-5"><h3 className="text-lg font-bold text-white">Interpretação ministerial</h3><div className="mt-4 space-y-3">{interpretation.map((item) => <article key={item.title} className="rounded-xl border border-white/10 bg-black/20 p-4"><h4 className="font-semibold text-white">{item.title}</h4><p className="mt-2 text-sm text-[#CBD5E1]">{item.narrative}</p>{item.examples?.map((example, index) => <p key={index} className="mt-2 border-l-2 border-brand pl-3 text-xs text-[#94A3B8]"><strong className="text-white">{example.pessoa}</strong> · {example.musica}{example.evidence ? ` — “${example.evidence}”` : ''}</p>)}</article>)}</div></div><div className="rounded-2xl border border-brand/20 bg-gradient-to-br from-brand/10 to-navy-900 p-5"><h3 className="text-lg font-bold text-white">Plano de ações ancorado</h3><div className="mt-4 space-y-3">{actions.map((item) => <article key={`${item.title}-${item.source}`} className="rounded-xl border border-brand/20 bg-black/20 p-4"><h4 className="font-semibold text-white">{item.title}</h4><p className="mt-2 text-sm text-[#CBD5E1]">{item.action}</p><p className="mt-3 text-xs text-brand">Por quê: {item.rationale}</p><code className="mt-2 block text-[10px] text-[#64748B]">{item.source}</code></article>)}</div></div></section>
}
...
function AssociationSummaryPanel({ items, correlations }: { items: SpiritualSummary['associations']; correlations: NonNullable<SpiritualSummary['correlations']> }) {
  return <div className="space-y-5"><section className="rounded-2xl border border-white/[0.08] bg-navy-900 p-5"><h3 className="text-lg font-bold text-white">Heatmap de correlações relevantes</h3><p className="mt-1 text-sm text-[#94A3B8]">Só são destacadas diferenças de pelo menos 15 p.p. em segmentos com 3 ou mais indicações.</p><div className="mt-4 grid gap-2 sm:grid-cols-2 lg:grid-cols-3">{correlations.filter((item) => item.relevant).map((item) => <div key={`${item.segment}-${item.value}-${item.dimension}-${item.polarity}`} className={`rounded-xl border p-3 ${item.difference > 0 ? 'border-emerald-400/30 bg-emerald-400/10' : 'border-rose-400/30 bg-rose-400/10'}`}><p className="text-sm font-semibold text-white">{item.segment}: {item.value}</p><p className="mt-1 text-xs text-[#CBD5E1]">{item.dimension} · {item.polarity}</p><p className="mt-2 text-lg font-bold text-white">{item.difference > 0 ? '+' : ''}{item.difference} p.p.</p><p className="text-[11px] text-[#94A3B8]">{item.segmentPercentage}% vs. {item.baselinePercentage}% · n={item.sampleSize}</p></div>)}{!correlations.some((item) => item.relevant) && <p className="text-sm text-[#94A3B8]">A amostra ainda não atingiu o critério mínimo de relevância.</p>}</div></section><section className="rounded-2xl border border-white/[0.08] bg-navy-900 p-5"><h3 className="font-bold text-white">Coocorrências de tema e emoção</h3><div className="mt-4 grid grid-cols-2 gap-2 sm:grid-cols-4">{items.map((item) => <div key={`${item.source}-${item.target}`} className="flex min-h-28 flex-col justify-between rounded-xl border border-brand/20 p-3" style={{ backgroundColor: `rgb(14 165 233 / ${Math.min(.08 + item.count * .08, .5)})` }}><p className="text-sm font-semibold text-white">{item.source} × {item.target}</p><p className="text-xs text-[#CBD5E1]">{item.count} ocorrência{item.count === 1 ? '' : 's'}</p></div>)}</div></section></div>
}
